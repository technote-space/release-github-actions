...
const cloneForBranch = (pushDir, branch, context) => __awaiter(this, void 0, void 0, function* () {
    ...
    yield execAsync(`git -C ${pushDir} clone --quiet --branch=${branch} --depth=1 ${url} .`, true, 'git clone');
});
...
const execAsync = (command, quiet = false, altCommand = null) => new Promise((resolve, reject) => {
    if (quiet && 'string' === typeof altCommand)
        signale_1.default.info(`Run command: ${altCommand}`);
    if (!quiet)
        signale_1.default.info(`Run command: ${command}`);
    child_process_1.exec(command + (quiet ? ' > /dev/null 2>&1' : ''), (error, stdout) => {
        if (error)
